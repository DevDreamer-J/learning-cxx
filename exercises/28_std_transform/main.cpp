...
std::string to_s(int number) {
    return std::to_string(number * 2);
}
int main(int argc, char **argv) {
    ...
    //std::string to_string(int& number) {
        //return std::to_string(number * 2);
    //}
    std::vector<std::string> ans = std::vector<std::string>(5);
    
    std::transform(val.cbegin(),val.cend(),ans.begin(),to_s);
